import {
  URBAN_HORRORS_CONTENT_VERSION,
  URBAN_HORRORS_DEFINITIONS,
  URBAN_HORRORS_MODULE_ID
} from "./content.js";

const MODULE_ID = URBAN_HORRORS_MODULE_ID;
const CORE_MODULE_ID = "pf2e-affliction-forge";
const PROVIDER_ID = "urban-horrors";
const LIBRARY_ID = "urban-horrors.core";
const PACK_NAME = "affliction-forge-urban-horrors";
...
async function createManagedPack() {
  ...
  try {
    return await CompendiumCollection.createCompendium({
      name: PACK_NAME,
      label: localize("PF2E_AFFLICTION_UH.Module.Library", "Urban Horrors"),
      type: "Item",
      package: "world",
      packageName: "world",
      system: "pf2e"
...
function addonFlags() {
  return {
    managed: true,
    contentVersion: URBAN_HORRORS_CONTENT_VERSION,
...
async function syncManagedPack(api, pack) {
  ...
  const creates = [];
  const updates = [];
  for (const definition of URBAN_HORRORS_DEFINITIONS) {
...
function registerProvider(api) {
  ...
  api.providers.register({
    id: PROVIDER_ID,
    label: localize("PF2E_AFFLICTION_UH.Module.Title", "PF2E Affliction Forge: Urban Horrors"),
    moduleId: MODULE_ID,
    version: URBAN_HORRORS_CONTENT_VERSION,
    metadata: {
      themes: ["urban", "intrigue", "affliction"],
    ...
    libraries: [{
      ...
      description: localize("PF2E_AFFLICTION_UH.Module.LibraryDescription", "Original urban afflictions with semantic Creature Forge tags."),
      metadata: {
        themes: ["urban", "intrigue", "affliction"],
        semanticTagContract: "1.0.0",
        creatureForgeReady: true,
...
async function initialize(api) {
  ...
  console.info(`${MODULE_ID} | Content synchronized`, {
    ...result,
    total: URBAN_HORRORS_DEFINITIONS.length,
    collection: PACK_COLLECTION,
    version: URBAN_HORRORS_CONTENT_VERSION
  });
}
...
Hooks.once("pf2eAfflictionForgeReady", (api) => {
  initialize(api).catch((error) => {
    ...
    globalThis.ui?.notifications?.error?.(
      localize("PF2E_AFFLICTION_UH.Module.SyncFailed", "Urban Horrors could not synchronize its library. See console.")
    );
  });
});
